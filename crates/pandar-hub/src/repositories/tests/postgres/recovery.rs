    ...
    let source = jobs
        .create_print_job(CreatePrintJob {
            ...
            artifact: crate::repositories::PrintArtifactInput {
                ...
                size_bytes: 42,
                storage_path: format!("{}/artifact-1/plate.3mf", tenant.id),
                metadata_json: None,
            },
    ...
    let completed_source = jobs
        .create_print_job(CreatePrintJob {
            ...
            artifact: crate::repositories::PrintArtifactInput {
                ...
                filename: "finished.3mf".to_string(),
                content_type: "model/3mf".to_string(),
                size_bytes: 84,
                storage_path: format!("{}/artifact-2/finished.3mf", tenant.id),
                metadata_json: None,
            },
            options: crate::repositories::PrintExecutionOptions {
                plate_id: 1,
...
async fn postgres_job_metadata_round_trips_and_reuses_artifact_when_configured() {
    ...
    let source = jobs
        .create_print_job(CreatePrintJob {
            ...
            agent_id: agent.id,
            artifact: crate::repositories::PrintArtifactInput {
                id: "artifact-metadata".to_string(),
                filename: "metadata.3mf".to_string(),
                content_type: "model/3mf".to_string(),
                size_bytes: 128,
                storage_path: format!("{}/artifact-metadata/metadata.3mf", tenant.id),
