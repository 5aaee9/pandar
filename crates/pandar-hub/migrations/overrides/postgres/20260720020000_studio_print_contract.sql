...
UPDATE jobs
SET plate_index = CASE
    WHEN jsonb_typeof(commands.payload_json::jsonb -> 'plate_id') = 'number'
         AND (commands.payload_json::jsonb ->> 'plate_id') ~ '^[0-9]+$'
    THEN CASE
        WHEN (commands.payload_json::jsonb ->> 'plate_id')::numeric BETWEEN 1 AND 2147483647
        THEN (commands.payload_json::jsonb ->> 'plate_id')::integer
        ELSE 1
    END
    ELSE 1
END
...
WITH ranked AS (
    SELECT id,
           ROW_NUMBER() OVER (
               ...
               ORDER BY created_at::timestamptz ASC, id ASC
           ) AS studio_submission_id
    FROM jobs
)
UPDATE jobs
SET studio_submission_id = ranked.studio_submission_id
FROM ranked
