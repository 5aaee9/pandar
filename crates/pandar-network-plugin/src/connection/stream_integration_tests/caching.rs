...
fn upgrade_auth_rejection_rejects_auth_once_without_connectivity_loss() {
    ...
    while std::time::Instant::now() < deadline {
        let result: PluginConnectionResult =
            unsafe { pandar_plugin_connection_take_transition(session) };
    ...
    let result = unsafe { pandar_plugin_connection_take_transition(session) };
...
fn account_epoch_change_fences_frames_from_the_previous_generation() {
    ...
    assert_eq!(0, unsafe {
        pandar_plugin_connection_set_account_epoch(session, 9)
    });
    scripts
        .send(Some(Script::Frames(snapshot_script(&[DEVICE_B_ONLINE]))))
        .unwrap();
