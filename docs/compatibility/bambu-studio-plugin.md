...
`02.07.01`, `02.08.00` adds the `bambu_network_bind` model argument, and `02.08.01` adds
`PrintParams::slicer_uid` plus `bambu_network_sync_ams_filaments`. AMS sync retains its exact
by-value ABI while returning the explicit redacted unsupported result; it does not authorize Bambu
cloud filament synchronization. These are compile-time C++ ABI differences, so one binary cannot
...
Installed Studio versions match the first three components: for example, `02.08.01.55` selects the
`02.08.01` artifact. Unknown ABI series fail closed.
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Windows x64                         | Catalog ABI series `02.06.00` through `02.08.01`, C++17, Release `/MD`, MSVC STL/UCRT                         | `x86_64-pc-windows-msvc`, MSVC C++17, `/MD`, `_ITERATOR_DEBUG_LEVEL=0`                       | Native Windows x64 runner per ABI series                            | ABI-series bundle/hook contract is configured; current native packages and real Studio runs remain `untested`                                                        |
| Linux x86_64                        | Catalog ABI series `02.06.00` through `02.08.01`, native C++17, libstdc++/glibc                                | `x86_64-unknown-linux-gnu`, native C++17 and an AppImage-compatible libstdc++/glibc baseline | Native Linux x86_64 runner per ABI series                            | current six-series packages and real Studio runs remain `untested`; final16 is historical Beta evidence only                                                         |
| macOS arm64                         | Catalog ABI series `02.06.00` through `02.08.01`, C++17, AppleClang/libc++                                     | Native `aarch64-apple-darwin`, AppleClang/libc++                                            | Apple Silicon `macos-26` runner                                      | release workflow is configured; historical local `02.07.01.62` arm64 package/load evidence does not cover the full six-series matrix                                 |
| macOS x86_64                        | Catalog ABI series `02.06.00` through `02.08.01`, C++17, AppleClang/libc++                                     | Targeted `x86_64-apple-darwin`, AppleClang/libc++                                           | Apple Silicon `macos-26` runner under Rosetta 2                      | release workflow is configured; target-architecture CI execution and real Studio evidence remain `untested`                                                          |
