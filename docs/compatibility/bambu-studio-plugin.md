...
Pandar builds a separate plugin for each supported Bambu Studio ABI series with upstream Boost
`1.84.0`:

| ABI series | Reference Studio | Studio commit                              | Source agent / reported       | Export contract                      |
| ---------- | ---------------- | ------------------------------------------ | ----------------------------- | ------------------------------------ |
| `02.06.00` | `02.06.00.51`    | `b506005bc4ee62124e24bf00e0f58656db3646a6` | `02.06.00.50` / `02.06.00.99` | 103 network + 21 File Transfer = 124 |
| `02.06.01` | `02.06.01.55`    | `6eb52d6ac75e32ba2116239c1d756d913053f364` | `02.06.01.50` / `02.06.01.99` | 108 network + 21 File Transfer = 129 |
| `02.07.00` | `02.07.00.55`    | `4410c27fb15d57b29fbb1dbebc6edea11a091137` | `02.06.01.50` / `02.07.00.99` | 108 network + 21 File Transfer = 129 |
| `02.07.01` | `02.07.01.57`    | `3f126b717ed1f10fee0f32f05ed9731808d0c8bb` | `02.07.01.51` / `02.07.01.99` | 108 network + 21 File Transfer = 129 |
| `02.08.00` | `02.08.00.50`    | `a78684a11de4abddad9a6d19eeb75a6a1d2e82a5` | `02.08.00.53` / `02.08.00.99` | 108 network + 21 File Transfer = 129 |
| `02.08.01` | `02.08.01.55`    | `ba049f6a2e08c3b6033660bb84da80c08722974b` | `02.08.01.52` / `02.08.01.99` | 109 network + 21 File Transfer = 130 |
| `02.08.02` | `02.08.02.61`    | `926a7192574bcb9b3a732e1ec59a46d79cb45466` | `02.08.02.54` / `02.08.02.99` | 110 network + 21 File Transfer = 131 |

The five filament-cloud exports begin in `02.06.01`, `PrintParams::svc_context` begins in
`02.07.01`, `02.08.00` adds the `bambu_network_bind` model argument, `02.08.01` adds
`PrintParams::slicer_uid` plus `bambu_network_sync_ams_filaments`, and `02.08.02` appends
`PrintParams::queue_plate_id` plus `bambu_network_sync_slot_mappings`. The queue field is retained in
its exact by-value layout. AMS and slot-mapping synchronization return explicit redacted unsupported
results (`-32` and `-33`) and do not authorize Bambu cloud filament or slot synchronization. These are
compile-time C++ ABI differences, so one binary cannot
safely serve every series. `studio-abi-profiles.json` drives build selection, pinned source
verification, export counts, installer matching, Windows hook gates, and release validation.
Installed Studio versions match the first three components: for example, `02.08.02.61` selects the
`02.08.02` artifact. Unknown ABI series fail closed.
The plugin reports `<abi-series>.99`, matching `open-bamboo-networking` and Studio's product-version
...
Final16 remains historical Linux compatibility evidence for Public Beta `02.08.01.55`. It includes the pinned Studio `fun` bit
...
2.8 artifacts must be rebuilt from the active ABI-series catalog entry and pass the same series-aware
source, package, and exact-Studio gates.
...
ABI series are compatibility build contracts, not a guarantee that every later Studio patch with the
same first three version components has been loaded. Compatibility is recorded per exact Studio build,
OS, architecture, and native C++ ABI; evidence for one ABI series or exact Studio build does not cover
another series or untested patch build.
...
Do not reuse ABI conclusions across ABI series or exact Studio builds.
...
| ----------------------------------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Windows x64                         | Catalog ABI series `02.06.00` through `02.08.02`, C++17, Release `/MD`, MSVC STL/UCRT | `x86_64-pc-windows-msvc`, MSVC C++17, `/MD`, `_ITERATOR_DEBUG_LEVEL=0`                       | Native Windows x64 runner per ABI series        | ABI-series bundle/hook contract is configured; current native packages and real Studio runs remain `untested`                          |
| Linux x86_64                        | Catalog ABI series `02.06.00` through `02.08.02`, native C++17, libstdc++/glibc       | `x86_64-unknown-linux-gnu`, native C++17 and an AppImage-compatible libstdc++/glibc baseline | Native Linux x86_64 runner per ABI series       | current seven-series packages and real Studio runs remain `untested`; final16 is historical Beta evidence only                         |
| macOS arm64                         | Catalog ABI series `02.06.00` through `02.08.02`, C++17, AppleClang/libc++            | Native `aarch64-apple-darwin`, AppleClang/libc++                                             | Apple Silicon `macos-26` runner                 | release workflow is configured; historical local `02.07.01.62` arm64 package/load evidence does not cover the full seven-series matrix |
| macOS x86_64                        | Catalog ABI series `02.06.00` through `02.08.02`, C++17, AppleClang/libc++            | Targeted `x86_64-apple-darwin`, AppleClang/libc++                                            | Apple Silicon `macos-26` runner under Rosetta 2 | release workflow is configured; target-architecture CI execution and real Studio evidence remain `untested`                            |
...
| 02.07.01.62    | macOS   | arm64        | `libpandar_network_plugin.dylib` + `libpandar_bambu_source.dylib` | local archive `4f5d2255839ea75f8a6d78e6729d91b92a9becc935d1b62946d6cb8c02a87a41`                   | 2026-08-01 | `passed`   | `untested`   | `untested`       | `untested`     | `untested` | `untested` | `untested` | `untested`       | `untested` | `untested`      | The installed official stable app reported version `2.7.1.62`, reached its normal home UI as an ARM64 process, and mapped both exact dylibs from `BambuStudio/plugins`; plugin SHA-256 `a43a10610421a34c5b2d4e49c4ededb4aa9d8a526474c69e98cf98327fac3930`, companion SHA-256 `34d9780ad48db45de4065e238bba735c43d0bdbcf54a5a8dd79e7bb3a49e2687`. Module-certificate validation was disabled only in the isolated test configuration because the local artifacts are unsigned. No sign-in, token, Hub/Agent, printer, print/control, firmware, or hardware path was exercised. Original Studio plugins, config, installer backup state, and trace length were restored after the test.                                                                                                             |
...
| ---------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-01 | macOS arm64 official stable `02.07.01.62` module load                           | Install the passed local three-file archive into the stable plugin directory, launch through Computer Use, and verify the normal home UI, process architecture, process mappings, and exact file hashes                       | `passed`  | Archive SHA-256 `4f5d2255839ea75f8a6d78e6729d91b92a9becc935d1b62946d6cb8c02a87a41`; ARM64 PID `74129` mapped both packaged dylibs and reached the normal home UI. The test temporarily disabled module-certificate validation for the unsigned local artifacts. No authentication or hardware action was performed; the original plugin files, config, installer backup state, and trace length were restored and hash-checked.                                                                                                       |
...
| Pinned Bambu Studio `02.08.02.61` native ABI contract                                                        | Compiles the exact upstream declarations and Print/AMS/slot-mapping/FT layouts against Boost 1.84.0, inspects the built export set, and invokes version, bind, print, AMS, slot-mapping, and all FT entrypoints through upstream function-pointer types.                                                                                                                           | `passed`      | 2026-08-31 Linux x86_64 local native caller: exact Studio commit `926a7192574bcb9b3a732e1ec59a46d79cb45466`, 110 network declarations, 21 FT declarations, exactly 131 exports, and all six `version,bind,print,ams,slot-mappings,ft` modes passed. This is automated native-caller evidence, not a packaged artifact or real Studio run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Historical truthful status, camera, and telemetry-presence probes                                            | Exercises Rust-owned status projection, `fun`/`cfg` masks, authoritative SD/chamber gates, the former explicit camera-unavailability boundary, no-cache/offline behavior, sequence-authoritative full refreshes, current-session liveness, model-presence preservation, and partial versus full Agent/Hub telemetry updates through the compiled ABI and both database adapters.   | `passed`      | 2026-07-21 Windows x64 final evidence: Agent authority/liveness 5/5, Hub route/presence/partial merge 5/5, plugin status plus pinned compiled Studio consumer 23/23, and final `on`/`flashing`/`off`/unknown lamp regression 1/1; fmt and strict plugin/Agent/Hub Clippy passed. Isolated Linux SSH previously ran the shared repository test against a real disposable PostgreSQL 16 container, Run ID `b48cf2e9-ffc5-4f86-b1e6-69d4c550b670`, then removed the container and `/tmp` source copy. Independent review returned `VERDICT: APPROVE`. This row predates the constrained local-camera implementation and remains historical evidence only.                                                                                                                                                                                                                                                                                                              |
| Constrained Studio local-camera probes                                                                       | Covers the exact A1/A1 Mini/P1S/A2L whitelist, current-session Agent capability gate, native TLS port-6000 authentication/frame bounds, Hub MJPEG route, one-use loopback relay token, exact local URL parser, 21-entry BambuSource ABI, and credential non-disclosure.                                                                                                            | `in_progress` | Targeted camera tests, 26/26 release-smoke, strict workspace Clippy, 1,923/1,923 workspace Nextest with one configured skip, and six x86*64 Linux Nix package/quality checks pass. The compiled Studio ABI probe verifies both camera callbacks exactly once for each of the four models, exact unique loopback URLs, and redaction. Linux `02.08.01` release inspection shows 130 network-plugin exports and exactly the sentinel plus 21 `Bambu*\*` companion exports. Packaged multi-platform candidates, a real Studio playback session, and real camera hardware remain pending.                                                                                                                                                                                                                                                                                                                                                                               |
...
Studio local camera is enabled only for normalized A1, A1 Mini, P1S, and A2L models. Status advertises
`ipcam_dev = "1"`, `liveview.local = "none"`, and `liveview.remote = "tutk"` only when an online
current Agent session advertises `StudioLocalCamera`; the Hub stream route repeats the model,
session, capability, authorization, and capacity checks. Every other or unknown model, stale session,
missing capability, and incomplete evidence remains unavailable.

For an eligible printer, both camera ABI functions invoke the callback with a random one-use
`bambu:///local/127.0.0.1?port=<loopback-port>&auth=<32-hex-token>` URL. The plugin relay uses the Hub
bearer internally, while Agent uses the printer host and access code to authenticate the native TLS
camera service on port 6000. Studio and BambuSource receive neither credential. BambuSource accepts
only this loopback URL shape, authenticates the relay token, and supplies bounded JPEG samples from
the Hub-mediated MJPEG stream. It does not accept direct printer hosts or implement Bambu
cloud/TUTK/Agora media. Automated evidence does not yet prove real Studio playback or real A1,
A1 Mini, P1S, or A2L camera hardware.
...
`pandar-bambu-source` companion exports `pandar_bambu_source_sentinel` plus the exact 21 `Bambu_*`
entrypoints used by Studio's local media path. The implementation is intentionally narrow: it accepts
only Pandar's authenticated loopback MJPEG URL, exposes bounded JPEG samples, and has no discovery,
cloud/TUTK/Agora, recording, MQTT, FTPS, or direct printer transport behavior.

A current candidate archive still has exactly three top-level files: the Pandar CLI, the network
plugin, and the companion. Release-smoke requires the pinned network-plugin contract plus the exact
21-entry local-camera BambuSource contract and sentinel. Prior stable, Final16, and Public Beta
packages with a sentinel-only companion remain valid historical evidence for their named snapshots,
not evidence for the current camera-capable package. New multi-platform packaging and real Studio
...
| Personal Process/Filament/Printer preset synchronization                                                                                     | `implemented; packaged Studio evidence pending` | The Hub provides authenticated user-owned storage and the plugin handles all six catalogue/drain/create/update/delete ABI calls with generation-scoped atomic caching. Compiled pinned-source tests cover every catalogued ABI series; no exact Studio/platform row is claimed until the packaged two-installation smoke gate passes.                                                   |
| Other cloud settings, cloud filament/AMS catalogue, MakerWorld/HMS, user messages/task reports, extra headers, certificate and init-log APIs | `unsupported`                                   | Pandar has no owned implementation for these remaining Studio cloud surfaces, so each returns its versioned disposition.                                                                                                                                                                                                                                                                |
