...
- Bambu LAN printers present printer-local/self-signed TLS certificates, including X.509 v1 certificates that rustls/webpki does not parse. The runtime agent uses a Bambu-specific verifier for MQTT, FTPS, and BRTC that accepts the printer certificate, extracts its public key with `x509-parser`, and keeps TLS handshake signature verification through rustls' AWS-LC algorithms. This policy is isolated to agent-to-printer LAN transports and does not apply to hub-facing HTTP/gRPC TLS.
