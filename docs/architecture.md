...
- The runtime agent applies one Bambu-specific TLS verifier to MQTT, FTPS, and BRTC. It requires the certificate CN to equal the configured serial, verifies chains against Studio's bundled Bambu printer CA set without requiring a DNS/IP SAN, and keeps TLS handshake signature verification through rustls' AWS-LC algorithms. Leaf-only certificates whose issuer is absent from the bundle require an operator-configured per-serial SHA-256 leaf pin through `PANDAR_BAMBU_CERTIFICATE_SHA256_PINS`; pin matching and certificate validity are fail-closed. This policy is isolated to agent-to-printer LAN transports and does not apply to hub-facing HTTP/gRPC TLS.
