...
- Prepared v0.1.3 as the focused patch release for the Studio sign-in public auth-status fix. Updated Rust, Node, Nix, Android, Helm, workflow fallback, generated deployment docs, release instructions, and lockfile version declarations together. Local release gates passed, including 1,971 workspace tests, 445 Web tests, 11 Auth tests, 26 release-smoke tests, Clippy, production builds, Helm/deployment checks, and Nix Web/Auth builds. Next: tag v0.1.3 and verify every published artifact.
