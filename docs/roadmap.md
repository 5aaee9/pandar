...
- Checked live soak prerequisites on 2026-06-24: local PostgreSQL binaries are available, but `tools/scaled-artifact-smoke` has no live mode and no disposable NATS/object-storage endpoint or credentials are configured, so live PostgreSQL+NATS+object-storage soak remains blocked.
