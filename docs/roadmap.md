...
- Locked the Bambu Studio **Synchronize personal data** RED contract at the compiled dynamic-library ABI seam. A source-pinned fixture is configured to compile against every catalogued Studio commit (`02.06.00` through `02.08.01`), resolve all six preset exports, verify their exact STL/callback signatures, and record the current explicit `unsupported_cloud_settings` disposition without implementing cloud behavior. It also records current callback non-invocation, output-map preservation/clearing, HTTP 501, and invalid-handle behavior so the next implementation slice has an executable boundary; successful catalogue, progress, cancellation, and drain semantics remain GREEN targets for the later mock-Hub implementation. Next: add the dual-backend Personal preset repository and durable owner clock before changing these operations to handled.
