...
- Bambu machines expose a second capability bitmap, `print.fun2`, beside the classic `print.fun` bitmap. Studio derives `is_support_print_with_emmc = fun2 bit 0` (`DeviceManager.cpp`) and forwards it as the `try_emmc_print` print parameter; BRTC eMMC uploads over implicit TLS port 6000 then publish `print.project_file` with a `brtc://emmc/<file>` URL (see `reference/open-bamboo-networking` §8.08/§8.14). Secondary observations follow the same current-session trust fence as `fun` and the Studio projection masks them to bit 0 only.
- BrTC transport selection is deliberately capability-probe based, not model-based: Bambu machines without the :6000 tunnel refuse the connection immediately, so Pandar defaults every printable upload to BRTC and degrades to protected FTPS with the BRTC cause preserved. Avoid hardcoding model allowlists so new machines get day-0 support (issue #5).
...
- Fixed the create-invite dialog role display reset. After a successful `createJoinLink` server action, React 19 resets the uncontrolled form fields, so the role radios snapped back to `defaultChecked` (Viewer) even when the link was created as Tenant admin or Operator. The selected role is now tracked in component state and mirrored into each radio's `defaultChecked`, so the post-action form reset restores exactly the role the user picked (fully controlled `checked` does not survive React's form reset for radios, so the state drives `defaultChecked` instead). Dialog close and "Create another link" still remount the form and reset the selection to Viewer. A Web regression proves the chosen role stays checked after a successful create, and the test fails against the previous implementation. Web lint/typecheck and 451 tests pass. Next: none.

- Fixed the sustained MQTT report pump overflow→resync churn (issue #8, software scope). The Agent's pump overflow warning now names the printer serial and the number of dropped buffered reports, and the resync-on-loss contract is unchanged; a hermetic tracing-capture regression proves the warning names its printer. The Hub's agent reverse-stream handler no longer applies printer snapshots serially: snapshot payload validation stays on the ordered inbound loop (malformed snapshots still fail the stream fast, and command results, job print reports, and other non-snapshot events keep their ordering and fail-fast semantics), while the current-session aggregate transaction and event fanout apply in bounded concurrent tasks (at most 8 in flight, then the loop applies backpressure) that are drained before session disconnect. The transition lease is released right after the aggregate commit, so concurrent applications serialize only on the database current-session fence and stale snapshots are still rejected before commit. A Hub regression proves a snapshot stalled in its event fanout no longer blocks a subsequent snapshot on the same stream, and the existing replacement/fence regressions guard the atomic current-session aggregate per printer. Workspace Nextest 2,084 pass plus the PostgreSQL-parity filter (71 pass), clippy and fmt clean. Next: deploy Hub and Agent, then record the sustained-print evidence (zero pump overflow warnings, else the per-printer firmware transition-mutex hold-time audit) against the live run in #7.

- Projected the BRTC transport URL onto the Web job detail (issue #7 follow-up from the successful retry run). The Agent's print-project-file result already carried `uploaded_url` (`brtc://emmc/<file>` or the FTPS location); the Hub now persists it onto the jobs row inside the same successful-print-command transaction (parsed typed from the result JSON, untouched on failures) and exposes it as a required-nullable `command.uploaded_url` field through the Hub contract (SQLite and PostgreSQL migrations, regenerated TypeScript and Kotlin clients), and the dashboard job details panel renders it alongside the printer-side file path. Workspace Nextest 2,082 pass including the PostgreSQL-parity filter, web lint/typecheck/450 tests pass, clippy and fmt clean. Next: Studio-submitted print evidence (issue #7).

- Fixed the BRTC :6000 tunnel after the first physical H2D evidence run (issue #7) proved the rustls transport cannot negotiate with any real machine: the printer's nLocal TLS server supports TLS 1.2 with a static-RSA key exchange only (`TLS_RSA_WITH_AES_256_GCM_SHA384`), which rustls removed by design, so every rustls ClientHello variant (default/classic-groups/TLS 1.2-only/TLS 1.3-only) received a fatal HandshakeFailure while an OpenSSL client negotiated the machine profile. The BRTC tunnel TLS now dials through an OpenSSL-backed client (vendored `openssl` crate; the flake gains perl for the vendored build) with SNI set to the printer serial, and after the handshake runs the shared `BambuLanCertificateVerifier` on the presented chain — the identical CN-equals-serial, pinned-leaf, legacy-V1, and CA-chain rules as the MQTT and FTPS tunnels, with no Bambu frame (including the bblp login) sent before the certificate passes. A new hermetic regression anchors the machine profile: an OpenSSL peer restricted to TLS 1.2 with only `AES256-GCM-SHA384` completes the full login/setup handshake, which the rustls client could never do. The Agent's tracing default no longer swallows operational warnings (default level `info` when `RUST_LOG` is unset), so the #5 requirement that a BRTC failure keeps its complete redacted cause in the runtime warning now holds in real deployments. Workspace Nextest 2,081 pass including the PostgreSQL-parity filter, clippy and fmt clean. The FTPS fallback's 30-second deadline remains a pre-existing limitation on this machine and is recorded in #7. Next: rebuild/deploy the Agent, then re-run the no-USB H2D gate with Web and Studio submissions (issue #7).

- Projected the session-fenced eMMC print capability to Bambu Studio (issue #6). The authoritative Studio device response and the versioned event stream now carry the secondary `fun2` observation under the same current-session trust fence as `fun` (stale sessions, absent observations, and Agents without the device-features contract all stay dark), and the network plugin's status projection emits `print.fun2` masked to bit 0 only, so an eligible machine becomes `could_emmc_print` while every unrelated fun2 feature stays masked. The source-pinned compiled Bambu Studio contract exercises the real `get_flag_bits_no_border(fun2, 0)` slice over the plugin projection and proves an eligible machine is advertised while an ineligible one fails closed; the submitted print reaches Hub with `try_emmc_print` forwarded as `try_emmc_print=true` and the agent transport from #5 selects BRTC/eMMC. The unsupported Studio File Transfer ABI remains fail-closed. Next: physical no-USB H2D print evidence (issue #7).

- Routed printable print artifacts to the machine's built-in eMMC over BRTC (issue #5, software scope). Every `.gcode.3mf` upload now probes the implicit-TLS :6000 tunnel first with no model allowlist, so new machine models work on day 0; unsupported machines refuse the tunnel fast and the upload degrades to protected FTPS while a warning keeps the complete BRTC cause in the log and the combined failure context. A hermetic Bambu-machine peer bound to the production port 6000 covers TLS login, control setup, eMMC upload initialization (`type=model`, `storage=emmc`, path/total), multi-chunk transfer with the final lowercase file MD5, and the terminal `result=0` reply; the real transport now maps success to `FileUploadResult::brtc_emmc`, and the machine gateway wire contract proves an H2D job publishes `print.project_file` with the exact uploaded filename, uppercase artifact MD5, and `brtc://emmc/<filename>` URL. `pandar-core` no longer carries a BRTC model predicate. Next: expose the session-fenced eMMC capability to Studio (issue #6) so Studio submissions carry `try_emmc_print=true`, then run the physical no-USB H2D print evidence gate (issue #7).

- Published `v0.2.0` from commit `94442a59ff15bbf77754d6448d5675ae07c35f28` as the structural-quality and Bambu Studio `02.08.02` release. Release-commit Checks run `33387170398`, tagged Checks run `33391086895`, Release run `33391086754`, and Docker/Helm run `33391086733` passed; one macOS matrix row was rerun after a GitHub Runner DNS failure while installing Rust. The GitHub Release contains the exact 70-file seven-series matrix, all 35 downloaded SHA-256 sidecars verified, every payload has its exact three-file layout, and all seven downloaded Linux CLIs start. Hub/Web manifests were published with digests `sha256:191a3dad22092efdee25c096d92f6ba5a5b89ae0314978c33c9058e53630dd22` and `sha256:0035b0faff4ada146514b5287f8e264b44c2775520a978ad4fb66d982ddccfc3`; Helm chart `0.2.0` was published with digest `sha256:46cb396c0e4620e3ada9e170f863bea72776c65a5847c6cd5b55c9cb4de2503b`. Next: record exact real Studio `02.08.02.61` load and authenticated-session evidence separately from package validation.

- Upgraded `suppaftp` to `10.0.2` after the release-commit CI discovered `RUSTSEC-2026-0271` in `10.0.1`; the local, release-commit, and tagged Rust security audits pass, and all 14 protected-FTPS focused regressions plus strict workspace Clippy pass. Next: keep monitoring upstream FTPS security advisories without weakening protected-data-channel policy.

- Added Bambu Studio `02.08.02` as a first-class ABI series pinned to official Studio `02.08.02.61` at `926a7192574bcb9b3a732e1ec59a46d79cb45466`. The series reports `02.08.02.99`, preserves the appended by-value `PrintParams::queue_plate_id`, exports the exact new `bambu_network_sync_slot_mappings` signature, and returns the Rust-owned redacted `-33 unsupported_slot_mappings_sync` disposition without enabling Bambu cloud slot synchronization. The source-pinned native caller verifies 110 network plus 21 File Transfer exports and all six `version,bind,print,ams,slot-mappings,ft` modes; its print sink now establishes the production printer-event stream before dispatch instead of relying on the retired printer-list initialization path. Installer, release-smoke, hook bundle, type-layout, symbol-map, and prior-series regressions cover the catalog boundary. Next: record an exact real Studio `02.08.02.61` load before claiming platform compatibility.

- Synchronized Studio file-transfer callback replacement with active callback dispatch. Status and result callbacks now acquire per-registration activity leases before invoking foreign code without holding the slot lock; replacing or clearing atomically retires the old registration and waits for its leases, while recursive dispatch remains available. Result dispatch also retains the job handle through callback-triggered release. The FFI contracts prohibit waiting on replacement from inside the registration being retired and guarantee that the old context is quiescent when replacement returns, with two-thread replacement and recursive-dispatch regressions for both callback families. Next: preserve this quiescent-replacement contract for any future persistent Studio callback slot.

- Deleted all legacy Hub job-storage compatibility boundaries. `AppState::job_storage`, `JobStorageAlias`, the public `jobs::JobStorageConfig` re-export, and filesystem-only forwarding methods are gone; tests construct `FilesystemArtifactStorage` through the canonical artifacts module. `StoredArtifact` now returns one opaque `storage_key`, which upload persistence writes into the existing database `storage_path` column, instead of duplicating the same value in a second field. Next: use `artifact_storage()` and `ArtifactStorage` directly for backend-neutral byte-storage behavior rather than introducing resource-specific wrappers.

- Made every pointer-validity-dependent network-plugin Rust/C ABI entry explicitly unsafe without changing its C ABI. Raw byte, opaque-handle, pointer-bearing DTO, allocation-reclamation, callback-context, and output-pointer exports now carry `# Safety` contracts; pointer readers and cross-session/callback helpers are unsafe so each Rust call site must acknowledge the invariant. Pure scalar exports and the three Studio no-op destroy/free compatibility functions remain safe because they never access their pointer arguments. Stream/account/recovery tests were split into test modules to preserve the 400-line production limit, and the quality gate now recognizes `_tests.rs` files as tests. Next: keep new raw-pointer FFI unsafe from its first revision and convert borrowed inputs to owned Rust values at the boundary.

- Removed retired and internal network-plugin C FFI surfaces. The old printer polling export and observation callback type are deleted, as are the superseded connection/cache/account freshness predicates that now belong to coarse `PluginCore` operations. Stream-cache integration coverage moved inside the connection module and reads private projection state while continuing to exercise active Studio FFI delivery behavior, leaving no test-only production export. Next: delete superseded FFI entrypoints when their coarse replacement lands rather than retaining compatibility symbols without callers.

- Made OpenAPI-required nullable Kotlin fields semantically required at decode time. Code generation now emits nullable properties without defaults plus `@Required`, while Android JSON uses explicit nulls and omits only actual defaults; an omitted required nullable key now raises `MissingFieldException`, but an explicit JSON `null` still decodes. Generated DTOs and request-body regressions preserve optional-field omission on encode. Next: keep wire presence in the OpenAPI `required` list so future nullable fields inherit the same generated decode contract.

- Made the omitted Nix auth and pinned Studio contracts durable Checks gates. Nix quality now builds the auth migration, JWT, cookie, and Web redirect smoke derivations. A dedicated job checks out the pinned Bambu Studio source, fetches every ABI-catalogue commit, and runs `studio_print_contract_red`, `studio_projection_contract`, and `personal_presets` through the repository's Nextest groups instead of relying on a developer's ignored reference checkout. Local derivation builds, the three contract binaries, and actionlint/treefmt validate the gate. Next: add future packaged-contract derivations or pinned-source binaries to Checks when they are declared, rather than excluding them without a compensating job.

- Made Android unit tests and lint durable merge gates. The path-filtered Android workflow now runs `testDebugUnitTest` and `lintDebug` before calculating the release version code and assembling/uploading the release APK, so a successful package build can no longer mask Kotlin/JVM test or Android lint failures. Actionlint and the same local Gradle tasks verify the workflow command. Next: add future Android quality checks to this pre-assembly gate rather than relying on manual release validation.

- Hardened tagged publishing against reusing a prior branch Checks run for the tag-only version invariant. Both the GitHub Release quality gate and tagged Docker/Helm quality gate now check out the exact triggering SHA and run `scripts/check-release-version.sh` against `GITHUB_REF_NAME` themselves before accepting any same-SHA Checks result. Main-branch and manual Docker runs remain unchanged. Actionlint/treefmt plus positive and negative release-version script runs cover the gate. Next: keep tag-specific release invariants inside each publishing workflow even when general code-quality results are reused by commit SHA.

- Removed the production compatibility and artifact-command legacy fallbacks. Hub print payload deserialization and queued-command conversion now require a non-empty authenticated `artifact_download_path` for every storage backend; Agent admission rejects missing paths and always downloads through Hub HTTP, so the local artifact-root reader, configuration, Nix option, path resolver, adapter, and dependency are gone. Printer REST/WebSocket payloads no longer synthesize compatibility during deserialization, and the OpenAPI contract plus generated TypeScript/Kotlin clients require the canonical projection. Hub, Agent, Web, Android, codegen, and configuration regressions cover the strict boundaries. Next: evolve these current wire fields explicitly instead of adding omission defaults or local-storage fallbacks.

- Moved the remaining network-plugin freshness and account-state policy behind coarse Rust `PluginCore` operations. Printer refresh now hands the core directly to Rust, where the cached projection observation reserves and validates the firmware generation without a C++ freshness branch; request/model-task freshness combines connection, cache, firmware, and snapshot checks in Rust; heartbeat logout state comes from the Rust connection session; and streamed firmware synchronization is one core operation. The shim retains only mutex, callback, STL, and opaque-pointer adaptation. Unit, compiled ABI, and all-header negative boundary coverage prevent these decisions from returning to C++. Next: route future cross-session freshness checks through `PluginCore` rather than composing borrowed-session predicates in the shim.

- Made `pandar-bambu-source` close safe when Studio re-enters from the reader worker's stream-info callback. The serialized tunnel lifecycle now detects self-worker closure, transfers the session to a detached finalizer that owns the lifecycle and shared state, and lets that finalizer shut down the socket, join the reader, publish EOF, and wake blocked callers without crossing the FFI boundary with a panic. A Studio-ABI regression closes from the callback and proves the callback returns and the tunnel reaches `Closed`. Next: retain this deferred-finalizer pattern for any future reader-thread callback that can initiate teardown.

- Unified network-plugin runtime ownership behind one Rust `PluginCore` opaque handle. The core now constructs and destroys the account lifecycle, connection/stream cache, firmware callback/status session, account identity, and firmware observation sequence as one lifetime. C++ `Agent` retains only the core handle and obtains borrowed connection/firmware views for ABI/STL/callback adaptation; it no longer allocates, destroys, or independently stores the three Rust sessions. Account mutation/lifecycle drain calls accept the core directly, and freshness observation reservation moved from a C++ atomic into the Rust owner. Core teardown stops its workers and clears identity-scoped caches once, while the existing Studio exports and callback threading remain unchanged. Compiled Studio ABI and core freshness coverage preserve external behavior. Next: add future plugin runtime state only to `PluginCore` rather than another C++-owned opaque handle.

- Established one generated Hub client contract. `contracts/hub-client.openapi.json` is served at `/api/v1/openapi.json`, declares the shared REST response schemas and closed command/job/print statuses, and deterministically generates split TypeScript wire models plus Kotlin serialization DTOs/enums. Web server/browser reads now validate known response fields through the contract instead of unchecked `response.json()` casts while remaining additive-field compatible; Android's handwritten duplicate response records were removed and its domain mappers consume generated typed materials and status enums. `npm run check:hub-clients`, Hub route coverage, Web contract tests, Android decoder tests, CI, Nix source filtering, and the documented generation workflow keep source and clients synchronized. Next: evolve response fields in the contract before changing Hub projections.

- Deepened Android control and state ownership behind one application boundary. `PandarDataSource` exposes the sealed domain `PrinterControlIntent` path plus one atomic `PandarState` Flow for printers, jobs, and latest commands; the reducer no longer fans one update into three independently observable flows. Every REST call captures one immutable Hub/API/token context with an opaque incarnation epoch, cache writes and accepted commands are owner/revision/event-sequence fenced, session replacement atomically clears the domain snapshot, and WebSocket overflow requests context-safe REST resynchronization while directly recovering the bounded latest dropped command per printer. Detail command status is derived only from the domain snapshot; sampled event cursors make accepted-response versus terminal-event ordering deterministic, queued frames survive same-context socket reconnects, and older recovered results cannot regress newer commands. Printer, Jobs, and Detail ViewModels depend on the focused interface rather than `AppContainer`; their transient state and jobs are generation-owned, stale projections fail closed, and coroutine cancellation propagates. Store/session, accepted-vs-terminal command, overflow recovery, delayed success/error, and pure UI-projection regressions cover these seams. Next: generate the shared Web/Kotlin response contracts consumed by this boundary.

- Made Android own one reactive authenticated Hub session lifecycle. A typed, secret-redacting `HubSession` is created only from a complete secure Hub/tenant/token identity; `PrinterEventsRepository` consumes that identity as a Flow over a dedicated HTTP client with one authentication owner, atomically cancels stale sockets on token, tenant, Hub, sign-out, or manual-reconnect changes, and generation-fences both callbacks and buffered event application. Rejected identities cannot be manually redialed, while refresh/conditional durable invalidation settles independently of reconnect cancellation and cannot clear a concurrently installed token. `AppContainer` is the sole owner, `PandarApplication` and callers no longer manually start/stop the stream, and sign-out awaits durable token removal. Mock-WebServer regressions cover socket replacement, exact authorization, sign-out/reconnect, stale-frame rejection, and suspended exactly-once invalidation after HTTP 401. Next: deepen Android screen control/state ownership behind focused application use cases.

- Projected canonical printer capabilities from `pandar-core` through Hub printer REST/WebSocket payloads. The projection now owns normalized model identity, calibration modes/defaults, dual-nozzle support/layout, and chamber-fan support. Web dispatch, material routing/nozzle labels, and cooling controls consume that projection instead of maintaining model-alias matrices; current REST/WebSocket contracts require the projection and reject payloads that omit it. Core/Hub route regressions and focused Web tests cover the projection. Next: generate shared client response DTO/status contracts from the Hub API schema.

- Aligned Web and Android control intents with the canonical printer-operation vocabulary. Web axis intents now carry typed `x|y|z` movements instead of loose axis strings, while preserving the existing server-action form boundary. Android now owns a sealed domain `PrinterControlIntent` and domain `PrinterAxis`; UI/ViewModels no longer construct Retrofit DTOs, the repository has one exhaustive `control(printerId, intent)` seam, and Retrofit exposes one `/controls` method through a typed transport mapper. Web lint/typecheck/461 tests and Android control-shape/full unit tests preserve every JSON action and field. Next: generate shared response DTO/status contracts from the Hub API schema.

- Adapted the Bambu Studio network-plugin parser to the canonical `pandar-core::PrinterOperation`. Studio JSON and G-code now construct and validate the shared semantic type; the former plugin operation/axis/feature enums and limit table were replaced by a typed `PrinterOperationRequest` adapter that exists only to preserve the public REST `action` envelope (`light_on`, optional waits/axes, and rack field names). Parser, HTTP boundary, native print-error, dispatch, and all compiled Studio ABI probes preserve exact external behavior. Next: align Web and Android domain intents with the same action vocabulary.

- Moved the Rust printer-operation domain into `pandar-core`. One `PrinterOperation`/`PrinterAxis` model now owns persisted serde shape, action names, limits, AMS/H2C validation, and required-device-feature semantics; Hub repositories/routes only map core validation into repository errors, while Agent protobuf admission converts once into the same model and machine/MQTT code executes it directly. The duplicate Hub and Agent operation/axis/error-action enums and validation tables were deleted, preserving REST JSON, protobuf payloads, MQTT commands, result JSON, and all Hub/Agent regressions. Next: adapt the Studio parser and client transport contracts to this canonical semantic model.

- Aggregated Hub print creation into `PrintArtifactInput` and `PrintExecutionOptions`. New uploads and duplicate/reprint flows now share one `CreatePrintJob` shape, one job/artifact insertion path, one domain builder, and one `PrintProjectFilePayload` projection; the duplicate twenty-field field bags and payload implementations were deleted without changing persisted columns, REST multipart fields, or Agent wire JSON. Full SQLite coverage and focused PostgreSQL job/recovery contracts preserve calibration, AMS mapping, artifact reuse, and Studio submission behavior. Next: extend print semantics through the execution aggregate instead of adding new flat fields.

- Unified Hub retention behind a typed cleanup plan. Dry-run counts, artifact candidate capture, and every execute delete now consume the same category selection SQL and ordered bind list; the duplicated job, command, ticket, token, audit, and machine-event delete predicates were removed, while the job/artifact transaction still durably enqueues object deletion before commit. Existing SQLite and required PostgreSQL cleanup matrices prove dry-run/execute parity and retained-resource safety. Next: add future retention categories only through `CleanupPlan::selection`.

- Applied each Agent printer snapshot as one current-session database aggregate. The Hub now upserts the primary printer state, persists both Bambu feature bitmaps/session markers, and clears authoritative material state inside the same SQLite/PostgreSQL transaction before publishing the committed projection; stale sessions cannot enter the aggregate. Backend-paired regressions inject a material-delete failure after the printer and feature writes and prove every projection rolls back together, while success coverage proves the complete state commits together. Next: reuse this aggregate boundary when adding new snapshot-owned projections.

- Replaced source-layout architecture assertions with durable boundaries. Network-plugin tests now use compiled ABI/behavior probes for freshness, callback ordering, firmware generation, and model-task lifecycles, while source inspection is limited to negative thin-C++-shim policy bans. The 400-line workspace production-module gate moved out of `pandar-core` into the dedicated `pandar-quality` tool, with the same command documented for local use and wired into Nix and GitHub Checks across Rust, C/C++, Web, and Android Kotlin sources. Next: keep new architecture invariants at compiled public seams and add negative source policy only when C++ business logic is explicitly forbidden.

- Consolidated the Bambu Studio network plugin onto one process-wide Tokio runtime and one reusable Hub HTTP transport. Generic plugin calls, connection readiness, firmware, personal presets, Studio print/model-task traffic, camera relay setup, and the printer-event stream now share the same runtime/client pool, bounded response readers, redirect/retry policy, and URL-redacted request error boundary while retaining their operation-specific deadlines and public ABI behavior. Heartbeat delivery now defers while the account callback owns a transition, preventing an already-issued offline callback from running after the account fence is released; the compiled request-admission probe covers that ordering.

- Simplified the dashboard route composition by deleting the universal `DashboardViewContent` view switch and its twenty-field prop bag. Devices, Jobs, and Agents now render focused `DevicesView`, `JobsView`, and `AgentsView` components that accept only the data each route owns; route clients no longer compute or pass placeholder health, notification, admin, authentication, token, or audit state for unrelated views. Existing composition and interaction coverage now targets the focused view contracts directly. Next: continue moving shared model compatibility decisions out of route components and into the canonical Hub projection.

- Made PostgreSQL backend parity a required, parallel-safe Checks gate. Every Hub parity test now creates and migrates a unique PostgreSQL schema and drops it when the test ends, replacing shared full-table truncation and preserving reconnect coverage inside the same isolated schema. The Nix quality job provisions PostgreSQL and runs the complete `test(/postgres/)` Nextest filter without serializing print-error tests; local development documents the disposable-database and schema-creation requirement. Next: keep new backend-dependent repository and route behavior in the mandatory PostgreSQL filter.

- Added a Hub-owned periodic artifact lifecycle worker. Each tick reaps expired quota reservations and claims a bounded deletion batch through short dialect-aware transactions; five-minute leases prevent duplicate object deletion across concurrent SQLite/PostgreSQL Hub instances while keeping storage I/O outside database transactions. A failed object now records its redacted error and attempt count, releases its claim, and no longer blocks healthy queued objects. SQLite regressions cover poison-entry progress, concurrent exactly-once claims, and the batch bound, with paired opt-in PostgreSQL coverage. Next: keep the lifecycle worker active in deployment smoke tests and monitor persisted retry counts for backend-specific storage failures.

- Serialized the `pandar-bambu-source` tunnel lifecycle behind one `Closed` / `Running` / `Closing` state machine. Opening now commits the relay socket, frame receiver, and reader worker as one session; closing marks that session as closing, tears it down completely, and wakes any waiting reopen without holding the lifecycle lock across callbacks. A deterministic overlap regression pauses an open before connection, starts close at the lifecycle boundary, then proves the committed worker and socket are shut down rather than leaked. Next: run the packaged camera source against a supported Studio build and repeat rapid open/close cycling under a live relay.

- Fenced Hub command dispatch, acknowledgements, and results against the authoritative persisted Agent session. Every Agent-originated command transition now locks and verifies the current session row in the same SQLite/PostgreSQL transaction as the command/job write, stale sibling sessions receive an aborted transition without changing durable state, and new sessions publish a sibling-close signal for prompt local-registry cleanup. Repository regressions cover stale dispatch/completion rejection and linearization between an in-flight transition and session replacement. Next: apply the same aggregate current-session transaction to the remaining multi-write printer snapshot projection.

- Completed the 2026-08-30 code-quality repair wave across Hub, Agent, network plugin, Web, and Android. Artifact uploads now use committed expiring quota reservations and cleanup/clear atomically enqueue idempotent object deletion before draining storage, with shared SQLite/PostgreSQL migrations and backend-parity regressions. Hub persistence now uses one backend-rendered printer upsert, one personal-preset transaction finalizer, one transport-neutral job projection, and coherent Studio session snapshots. The network plugin moved account/session mutation and firmware generation ownership into Rust, preserves callback generations, classifies malformed H2C requests correctly, and bounds synchronous print polling with one absolute deadline. The Agent preserves causal machine errors through FFI/runtime boundaries, uses one Machine snapshot wire projection, and probes independent printers concurrently with an eight-probe freshness-fenced bound. Web route data now has canonical resource-owned caches, centralized mutation invalidation, route-local typed filters, and one exact Studio callback URL owner; Android reduces transport events once in the repository. Oversized Hub tests were split below 400 lines and the module-size guard now covers production Kotlin. The independently reviewed lane commits are now rebased and fast-forwarded on `main`; next finish the final full workspace and dual-backend software gates, then repeat packaged Studio and physical-printer smoke tests.

- Hardened the issue #4 protected-FTPS software path without changing its trust or downgrade policy. The Agent's X2D/P2S Rustls profile now uses TLS 1.2 session-ID-only resumption on the single `ClientConfig` shared by the implicit-FTPS control and passive data connections, so the data channel presents the exact control session ID instead of relying on RFC 5077 ticket resumption. A hermetic X2D/vsFTPd-compatible server test enables both ticket and session-ID mechanisms, reproduces `522` with the prior ticket preference, then covers login, `PBSZ 0`, `PROT P`, binary mode, passive `STOR`, exact control-session-ID reuse, finalize, and `SIZE`; a rejection case preserves the exact `522 SSL connection failed: session reuse required` cause at the typed `data_connection` phase. Model regressions keep branded X2D/N6 and P2S/N7 TLS-1.2-capped while H2D retains safe-default negotiation, and no `PROT C` path exists. Failed print transfers now carry a typed phase plus the Agent's complete redacted cause through command result persistence and the Studio job API, and the network plugin returns that detail instead of replacing it with only `job_failed`; Agent and Hub tests prove access-code redaction, SQLite plus opt-in PostgreSQL transition coverage, and the compiled Studio callback contract. This automated evidence does not claim the hardware gates: next rebuild/deploy the Agent, complete one real affected-X2D print without `522`/timeout, and repeat an H2D print regression.

- Landed the four architectural cleanups from the 2026-08-26 shim review. The Agent's hub-command admission now has one outcome: a typed `CommandAdmission` (valid / protocol reject) settles every illegal command — including firmware outer/inner id mismatch, missing `ExecuteFirmwareControl` payloads, and `HubCommand` messages without a command — as a rejected acknowledgement that keeps the reverse stream alive, so only queue or runtime failures (blocked event channel, worker teardown) may tear the stream down. The Agent's FTPS layer dropped its dead transfer-mode fallback architecture entirely: `TransferProtectionMode`, `TransferModeCache`, `transfer_attempt_order`, and `run_with_transfer_mode` are gone, `MachineFileTransfer` no longer threads a mode parameter, uploads/diagnostics call the protected transfer directly with `.context()`, and the error chain is no longer flattened into a string. The Hub's persistence layer no longer imports upward: `RequiredDeviceFeature` moved into `pandar-core` beside `BambuDeviceFeature` (the grpc module keeps only the proto wire mapping), and `PrintErrorAction` plus the build-plate recovery catalog (`plate_mismatch::supports` with its tests) moved into `pandar-core::print_error`, deleting `routes/printer_operations/plate_mismatch.rs` and leaving repositories → routes/grpc with no references. The network plugin's routing/scheduling policy converged into coarse-grained Rust FFI behind one `PluginDispatchBridge` vtable: `pandar_plugin_dispatch_message` owns the whole classified-message routing (firmware send/handoff, get_version/pushall status delivery with firmware overrides, H2C and printer-operation submission with admission and snapshot-currency fences), `pandar_plugin_dispatch_pending` owns one heartbeat iteration including the no-auth retry decision, plus dedicated dispatchers for direct local connect (preserving the LAN callback's JSON body), the firmware callback handoff window, and post-refresh drains; the C++ shim deleted `shim_status.hpp`, `shim_status_delivery.hpp`, and the inline routing in `shim_firmware.hpp`/`shim_abi_operations.hpp`, keeping only callback gates, the threads' wait/stop mechanics, and STL adapters. Coverage: new/updated admission and stream-lifecycle tests, workspace Nextest 2,006 pass including all compiled Studio ABI probes, clippy and rustfmt clean. Next: move the seven-way `mutation.action` application in `shim_account_transaction.hpp` behind a Rust-driven state-mutation adapter so the account transaction header shrinks to lock + callback plumbing.

- Unified the agent↔hub protocol into one `pandar-protocol` crate. The hub and the agent previously ran byte-identical `build.rs` files that each generated a private copy of the `pandar.agent.v1` types, then hand-maintained reciprocal field-by-field converters (firmware module/upgrade state, cooling, nozzle, device features) that could drift independently. The new crate owns the single tonic-prost generation, hosts both conversion directions for the pure mirrors (`proto_module`/`core_module`, `proto_upgrade_state`/`core_upgrade_state`), shares the agent's domain→wire mappers (`proto_cooling_system`, `proto_nozzle_system`, `proto_device_features`), adds the inverse `core_device_features` decode the hub previously inlined, and consolidates the wire-format round-trip tests (deduplicating a byte-identical test both crates carried). Adding a wire field now touches the proto plus one conversion module instead of mirrored copies in three places. Hub-side inbound converters that interleave validation with mapping stay in the hub as the trust boundary. Both crates dropped their build scripts, `tonic-prost`, and `prost` dependencies; `tools/scaled-artifact-smoke` protocol imports were migrated to the new crate (its remaining fixture drift predates this change). Coverage: workspace Nextest 2,007 pass, clippy and rustfmt clean.

- Fixed the five verified cluster-correctness bugs from the 2026-08-26 thermo-nuclear review (see [the fix log](plans/2026-08-26-cluster-correctness-bug-fixes.md)). The network-plugin build script now derives its `rerun-if-changed` header list by scanning `src/shim_*.hpp` instead of a hand-written list that named a nonexistent header and omitted seven real ones, so editing any shim implementation header recompiles the C++ object. Studio message dispatch classifies H2C auto-nozzle-mapping requests by their typed `print.command` envelope instead of a substring probe, so valid `gcode_line` operations that merely mention the command are no longer misrouted to an invalid-H2C result while malformed mapping requests still report the H2C kind. gRPC command results now fan out through the control plane (`publish_printer_event`) instead of local-only delivery, so WebSocket clients connected to other hub replicas receive them; `PrinterEventHub::publish_local` became the consumer-only `deliver_local`. Printer-event epoch invalidation is per tenant — a failed publish closes only that tenant's sockets while control-plane-wide failures still invalidate every tenant via `invalidate_all_epochs` — and the materials JSON scrubbing moved to `printer_events/materials.rs`, keeping the module under the 400-line limit. User role changes now run in dialect-aware write transactions with `FOR UPDATE` row locks on PostgreSQL, and `update_user_role_with_audit` rejects demoting the last tenant admin, closing both the sole-admin downgrade path and the concurrent double-admin removal race. Coverage: studio-message dispatch regression tests (gcode substring, valid/malformed H2C, status ordering), control-plane command-result delivery test, cross-tenant epoch-isolation test plus updated control-plane failure tests, and last-admin demotion/removal tests; workspace Nextest 2,008 pass, clippy and rustfmt clean.

- Fixed the six verified bugs from the 2026-08-26 quality review (see [the fix log](plans/2026-08-26-quality-review-bug-fixes.md)). The network-plugin `ft_job_get_result` Condvar predicate was inverted so unfinished jobs returned `FT_ETIMEOUT` without ever waiting; it now waits while the job is unfinished and reports the job's own error code on completion. The Android printer-event stream never produced its `Connected` outcome, so healthy reconnects compounded cold-start backoff toward 30 seconds; outcomes now distinguish traffic-carrying sessions (backoff resets) from empty ones, with the delay state extracted into a unit-tested `ReconnectBackoff`. The dashboard shell rendered action toasts only from a Zustand field that had no writer, so every server-action redirect's `?status=` result was silently dropped; the persistent shell now renders `ActionStatusToast` from the URL query and the dead store is deleted. The Jobs page granted management controls whenever `membership.role !== "viewer"`, including membership load failures and missing memberships; authorization policy moved into fail-closed `membership-policy.ts` helpers shared by Jobs, Users, and Agents, and Users' unreachable `adminLoadError` conjunction became reachable for real load failures. Hub log redaction replaced only the first occurrence of each secret key per line; repeated credentials such as two `access_code=` values in one message are now all redacted, and non-key-value occurrences no longer abort scanning. The Agent MQTT report queue silently dropped the oldest buffered report at capacity, losing incremental material patches with no signal; overflow now clears the buffer, emits an explicit overflow error that fails the forwarding consumer, and the existing retry loop resubscribes and publishes a fresh `pushall` for full resynchronization. Coverage: new redaction occurrence tests, plugin job-wait/timeout tests, Android `ReconnectBackoffTest`, shell `?status=` toast regression, and `membership-policy.test.ts`; workspace Nextest 2,002 pass, web lint/typecheck/456 tests pass, Android `:app:testDebugUnitTest` passes.

- Replaced Bambu Studio's two-second full printer-list polling with the versioned tenant printer-event stream from issue #3. The Hub now authorizes same-tenant `PluginStudio` tokens only for `projection=studio&version=1`, publishes an atomic complete snapshot followed by typed full-record upserts/removals through the existing tenant/control-plane boundary, shares the Studio printer projection with the HTTP list endpoint, and invalidates on lag or event-epoch gaps without changing the default Viewer/ticket contract. The Rust network plugin owns one account-fenced WebSocket, an atomic memory-only cache, selected/subscribed latest-wins delivery, streamed firmware observations, Ping/Pong and bounded reconnects; short flaps retain the cache without `-2`, while a five-second `/healthz` observation after 30 seconds distinguishes one real Hub-disconnect callback from one healthy-Hub `503 printer_event_stream_unavailable`/offline episode. Sign-out, account replacement, auth rejection, stale frames, removals, and destruction retain the existing generation/ticket/final-claim fences, and the C++ shim remains callback/lifecycle-only. Real Axum route tests and the compiled Studio ABI probe cover authorization, parity, initial/empty snapshots, no polling or stale replay, asynchronous changes, removal, recovery, both health outcomes, 401/403, keepalive, backoff, and account/destroy races. Next: deploy Hub support before the plugin and repeat the forced short-interruption versus real-outage smoke in a packaged supported Bambu Studio installation.

- Made the native Studio ABI gates deterministic in hermetic Nix Nextest runs. The Rust source filter now carries `.config/nextest.toml` into the sandbox, so the existing `studio-abi-probe` test group governs resource-heavy nested Cargo builds instead of launching them all at once. The freshness final-claim probe waits for an observed stream-cache commit before releasing its blocked firmware HTTP response, removing an invalid ordering assumption across the WebSocket and HTTP connections while continuing to exercise the production cache-generation and delivery-ticket fences. The request-admission probe now counts its own firmware refresh and waits for an explicit post-teardown completion marker, so the generic responder and a filtered mock-stop panic can no longer bypass its no-Hub-I/O assertions. The disposition-only probe holds a loopback Hub listener through Agent destruction, preventing its unrelated authenticated stream worker from racing the process against a closed default endpoint. Logout completion now wakes the status dispatcher after the remote revocation outcome, so the two-second automatic no-auth recovery delay starts after the explicit DELETE finishes instead of racing that DELETE's equal timeout; the timeout probe holds the Agent alive through its no-immediate-retry assertion. Next: keep new nested-build Studio ABI binaries in the governed Nextest group.
  The firmware callback integration assertion now measures its caller-observable one-to-two-second window from ABI return; the exact 1.1-second handoff anchor remains covered with synthetic `Instant`s, avoiding a sub-millisecond pre-return interval in the lower bound.
...
- Locked the Bambu Studio **Synchronize personal data** RED contract at the compiled dynamic-library ABI seam. A source-pinned fixture is configured to compile against every catalogued Studio commit (`02.06.00` through `02.08.02`), resolve all six preset exports, verify their exact STL/callback signatures, and record the current explicit `unsupported_cloud_settings` disposition without implementing cloud behavior. It also records current callback non-invocation, output-map preservation/clearing, HTTP 501, and invalid-handle behavior so the next implementation slice has an executable boundary; successful catalogue, progress, cancellation, and drain semantics remain GREEN targets for the later mock-Hub implementation. Next: add the dual-backend Personal preset repository and durable owner clock before changing these operations to handled.

- Specified Bambu Studio **Synchronize personal data** support for user-owned Process, Filament, and Printer presets. The documented design fixes the Studio ABI catalogue/cache contract, authenticated user ownership, no-auth exclusion, strictly increasing owner timestamps, dual SQLite/PostgreSQL persistence, privacy limits, rollout, and exact-version evidence gates. The implementation sequence is recorded in [the delivery plan](plans/2026-08-12-bambu-studio-personal-data-sync.md) against [the feature spec](specs/2026-08-12-bambu-studio-personal-data-sync.md).
...
- Published `v0.1.3` from commit `7a5fabc1ed119df1641e6116403ce9053e004d6f` as the focused patch release for the Studio sign-in public auth-status fix. Tagged Checks run `31479109340`, Release run `31479109376`, and Docker/Helm run `31479109343` passed. The GitHub Release contains 60 ABI-series-specific desktop files, and all 30 downloaded SHA-256 sidecars verified. Hub/Web manifests were pushed with digests `sha256:7dc30ec3c7affff6f2a92ce195decff08279a3bada10e01426540a113834240c` and `sha256:da950ed4f89eeeaa7b851ba20fcf9cc878a9d87c0ef7d879ae1f767c1a8c9ae1`; Helm chart `0.1.3` was pushed with digest `sha256:65287a106ee57bb745db70b5a42e90b01f6eba51b0a32efd2154bfa30c6c7797`. Next: validate the packaged plugin in an authenticated Studio session against Pandar Auth.

- Fixed the remaining Studio sign-in `Readiness check returned 404` regression from v0.1.2. `/readyz` is intentionally available only on the private observability listener, so the Hub now exposes a minimal public `/api/v1/auth/status` response containing only external-auth enabled/readiness booleans, and Pandar Web uses that endpoint instead. Focused Hub and Web regression tests cover configured and disabled auth without exposing private readiness details. Next: publish the fix and validate the packaged plugin against Pandar Auth.

- Published `v0.1.2` from commit `fa496229781b0d0f3c4f9f24a3e03b03eb01e190` for the single-Web-URL Studio login flow and Pandar Auth readiness fix. Tagged Checks run `31415669877`, Release run `31415669820`, and Docker/Helm run `31415669972` passed. The GitHub Release contains 60 ABI-series-specific desktop files, and all 30 downloaded SHA-256 sidecars verified. Hub/Web manifests were pushed with digests `sha256:76f7543601aa478ff131937fce7fc74f1b1774de960239170fdba764d2b5cfcc` and `sha256:23291aef5aab18d43595908e5760ebc73219173867a2f76e5c2eeb1f2d148de9`; Helm chart `0.1.2` was pushed with digest `sha256:149ad379e3652f538ac3af2025552aabd4bf180bc8237a5ff4ae43a3db006040`. Next: validate the packaged plugin in an authenticated Studio session.

- Simplified Bambu Studio plugin targeting to one Web URL. Pandar Web now publishes the Studio-reachable Hub URL through `/.well-known/pandar` (using `APP_PUBLIC_API_URL` when the server-side API address is internal), and the embedded plugin page discovers and stores it automatically instead of asking users for a second Hub URL. Changed the plugin sign-in page from `/healthz` to `/readyz`; the post-v0.1.2 follow-up above replaces that private-listener request with the public auth-status API. Focused Web and compiled plugin-local tests cover the URL-discovery regression. Next: validate the updated packaged plugin in an authenticated Studio session.
...
- The original local artifact-root reader and path-resolution flow was later retired; current agents require authenticated Hub artifact download paths and still reject unknown serials before artifact I/O.
