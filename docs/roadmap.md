...
- Upgraded `suppaftp` to `10.0.2` after the release-commit CI discovered `RUSTSEC-2026-0271` in `10.0.1`; the Rust security audit is clean apart from the existing allowed warnings, and all 14 protected-FTPS focused regressions plus strict workspace Clippy pass. Next: keep the release blocked until the updated commit's full remote gates pass.

- Prepared `v0.2.0` as the structural-quality and Bambu Studio `02.08.02` release. Rust, Node, Nix, Android, Helm, workflow fallback, generated deployment docs, release instructions, changelog, and lockfile version declarations move together. Local release gates passed: strict Clippy, rustfmt/treefmt, module-size, 2,073 workspace tests, 26 release-smoke tests, 450 Web tests, 11 Auth tests, generated-client checks, production builds, production dependency audit, Helm/deployment checks, and Nix Web/Auth builds. The preceding main-branch Checks and Docker/Helm workflows also passed for the release candidate source. Next: push the release commit, wait for its remote gates, tag `v0.2.0`, and verify every published artifact and checksum.

- Added Bambu Studio `02.08.02` as a first-class ABI series pinned to official Studio `02.08.02.61` at `926a7192574bcb9b3a732e1ec59a46d79cb45466`. The series reports `02.08.02.99`, preserves the appended by-value `PrintParams::queue_plate_id`, exports the exact new `bambu_network_sync_slot_mappings` signature, and returns the Rust-owned redacted `-33 unsupported_slot_mappings_sync` disposition without enabling Bambu cloud slot synchronization. The source-pinned native caller verifies 110 network plus 21 File Transfer exports and all six `version,bind,print,ams,slot-mappings,ft` modes; its print sink now establishes the production printer-event stream before dispatch instead of relying on the retired printer-list initialization path. Installer, release-smoke, hook bundle, type-layout, symbol-map, and prior-series regressions cover the catalog boundary. Next: produce target-OS packages and record an exact real Studio `02.08.02.61` load before claiming platform compatibility.
