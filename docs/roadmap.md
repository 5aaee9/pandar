...
- Prepared `v0.2.1` as the Studio server-selection restore release. Rust, Node, Nix, Android, Helm, workflow fallback, generated deployment docs, release instructions, changelog, and lockfile version declarations move together, and the deployment-security script's chart `appVersion` pin follows the release. Local release gates passed: strict Clippy, rustfmt/treefmt, 2,083 workspace tests, 26 release-smoke tests, 22 Studio ABI contract tests, 14 Studio plugin smoke tests, 449 Web tests, Auth tests and typechecks, production Web/Auth builds, production dependency audit, Helm and deployment checks, and the version-consistency script against the regenerated lockfiles and Nix package builds. Next: push the release commit, wait for its remote gates, tag `v0.2.1`, and verify every published artifact and checksum.
