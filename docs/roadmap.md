...
- Deleted the legacy Hub `JobStorageAlias` boundary. The sole route-test consumer now reads the canonical `ArtifactStorage::max_artifact_bytes` capability directly, so `AppState::job_storage`, the alias re-export, and its unused write/remove/readiness compatibility methods are gone. Next: use `artifact_storage()` directly for backend-neutral byte-storage behavior rather than introducing resource-specific forwarding wrappers.
