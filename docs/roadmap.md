...
- Completed Filament Track Switch passthrough to Bambu Studio: Agent material patches now preserve each AMS unit's normalized raw `info` bitmap through Hub events and plugin printer responses, while the Rust network-plugin telemetry maps installed/absent/unknown switch state to Studio's `print.aux` bit 29 contract and forwards only valid `0xE` switch routes with input A/B intact. Missing or malformed switch routes are omitted instead of being projected onto the right extruder, and the Studio ABI probe now verifies `aux=20000000` plus both switch-bound AMS bitmaps.
