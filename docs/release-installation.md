...
| --- | --- | --- | --- |
| `linux-amd64` | `blocked` | No release artifact exists yet for this target. | Create a tag/release artifact, then install the archive on a Linux x86_64 host, verify checksum, run `pandar --help`, inspect plugin replacement behavior, and record the row in `docs/compatibility/release-artifacts.md`. |
| `linux-arm64` | `blocked` | No release artifact exists yet for this target; plugin export reliability is still watched because the Linux `cdylib` export path is fragile on arm64. | Create a tag/release artifact, run release-smoke for the target, install on Linux arm64 hardware, and record export plus install evidence before treating it as usable. |
| `windows-amd64` | `blocked` | No release artifact exists yet for this target; artifacts are unsigned and may trigger platform warnings. | Create a tag/release artifact, install on Windows x86_64, verify checksum, run `pandar.exe --help`, replace the Studio plugin DLL for a controlled smoke, and record evidence. |
| `windows-arm64` | `blocked` | No release artifact exists yet for this target; artifacts are unsigned and may trigger platform warnings. | Create a tag/release artifact, install on Windows arm64, verify checksum, run `pandar.exe --help`, replace the Studio plugin DLL for a controlled smoke, and record evidence. |
| `macos-amd64` | `blocked` | No release artifact exists yet for this target; artifacts are unsigned and may trigger Gatekeeper warnings. | Create a tag/release artifact, install on Intel macOS, verify checksum, run `pandar --help`, replace the Studio plugin dylib for a controlled smoke, and record evidence. |
| `macos-arm64` | `blocked` | No release artifact exists yet for this target; artifacts are unsigned and may trigger Gatekeeper warnings. | Create a tag/release artifact, install on Apple Silicon macOS, verify checksum, run `pandar --help`, replace the Studio plugin dylib for a controlled smoke, and record evidence. |
