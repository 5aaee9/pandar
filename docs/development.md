...
Server-selection persistence and precedence: a target server saved through the local `/config` endpoint is recorded next to the Studio login as `pandar-plugin-server-selection.json` — the typed Web URL plus its discovered canonical Hub identity, written with the same durable replace-and-confirm discipline as the login file. On the next clean launch, with none of `PANDAR_PLUGIN_HUB_URL`, `APP_API_URL`, `PANDAR_PLUGIN_FRONTEND_URL`, or `APP_BASE_URL` set, the plugin restores the saved Web and Hub URLs before it evaluates the persisted Studio login, so a valid same-Hub login is visible through the Studio ABI without another sign-in and the local sign-in page shows the selected server again. Explicit URL environment configuration always outranks the saved manual selection, a credential belonging to a different Hub is never restored (it is cleared when the runtime switches Hubs), and a malformed or unreadable selection file is ignored — closed — with its diagnostic logged while the login file itself is unaffected.
