...
This runbook prepares and publishes a Pandar release. The `v0.2.0` tag publishes desktop archives, Hub/Web container images, and the Helm chart. It does not publish an Android APK.
...
git status --short
bash scripts/check-release-version.sh 0.2.0
```
...
bash scripts/check-release-version.sh 0.2.0
git tag -a v0.2.0 -m "Pandar 0.2.0"
...
- `Docker`, which publishes `hub:v0.2.0`, `web:v0.2.0`, and Helm chart `0.2.0` after Checks succeeds.
...
```bash
gh run list --commit "$(git rev-list -n 1 v0.2.0)" --limit 20
gh release view v0.2.0
gh release download v0.2.0 --dir /tmp/pandar-v0.2.0
helm show chart oci://ghcr.io/projectpandar/pandar/chart/pandar --version 0.2.0
docker pull ghcr.io/projectpandar/pandar/hub:v0.2.0
docker pull ghcr.io/projectpandar/pandar/web:v0.2.0
```

The `v0.2.0` release is expected to have 70 desktop files: each ABI series has four
