...
This runbook prepares and publishes a Pandar release. The `v0.1.2` tag publishes desktop archives, Hub/Web container images, and the Helm chart. It does not publish an Android APK.
...
git status --short
bash scripts/check-release-version.sh 0.1.2
```
...
bash scripts/check-release-version.sh 0.1.2
git tag -a v0.1.2 -m "Pandar 0.1.2"
...
- `Docker`, which publishes `hub:v0.1.2`, `web:v0.1.2`, and Helm chart `0.1.2` after Checks succeeds.
...
```bash
gh run list --commit "$(git rev-list -n 1 v0.1.2)" --limit 20
gh release view v0.1.2
gh release download v0.1.2 --dir /tmp/pandar-v0.1.2
helm show chart oci://ghcr.io/projectpandar/pandar/chart/pandar --version 0.1.2
docker pull ghcr.io/projectpandar/pandar/hub:v0.1.2
docker pull ghcr.io/projectpandar/pandar/web:v0.1.2
```

The `v0.1.2` release is expected to have 60 desktop files: each of the six ABI series has four
