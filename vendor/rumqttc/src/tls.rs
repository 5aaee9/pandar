    ...
    let config = match tls_config {
        TlsConfiguration::Simple {
            ...
            let mut config = if let Some(client) = client_auth.as_ref() {
                let certs = CertificateDer::pem_reader_iter(Cursor::new(client.0.clone()))
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
                if certs.is_empty() {
                ...
                let key = PrivateKeyDer::from_pem_reader(Cursor::new(client.1.clone()))
                    .map_err(|_| Error::NoValidKeyInChain)?;
