package zip.iptables.pandar.android.data.remote

import kotlinx.serialization.encodeToString
import org.junit.Assert.assertEquals
import org.junit.Assert.assertFalse
import org.junit.Test
import zip.iptables.pandar.android.data.remote.dto.toRequest
import zip.iptables.pandar.android.domain.model.PrinterAxis
import zip.iptables.pandar.android.domain.model.PrinterControlIntent
import zip.iptables.pandar.android.domain.model.moveAxisIntent

class ControlsBodyShapeTest {

    private val json = appJson

    private fun encode(intent: PrinterControlIntent) = json.encodeToString(intent.toRequest())

    @Test fun pause_is_minimal() =
        assertEquals("""{"action":"pause"}""", encode(PrinterControlIntent.Pause))

    @Test fun no_polymorphic_discriminator_leaks() {
        val body = encode(PrinterControlIntent.Pause)
        assertFalse("type discriminator leaked", body.contains("\"type\""))
    }

    @Test fun resume_is_minimal() =
        assertEquals("""{"action":"resume"}""", encode(PrinterControlIntent.Resume))

    @Test fun stop_is_minimal() =
        assertEquals("""{"action":"stop"}""", encode(PrinterControlIntent.Stop))

    @Test fun home_all_axes_is_explicit() =
        assertEquals(
            """{"action":"home","axes":[]}""",
            encode(PrinterControlIntent.Home()),
        )

    @Test fun every_axis_sign_and_step_maps_to_the_exact_request() {
        val cases = listOf(
            Triple(PrinterAxis.X, -10.0, 3000),
            Triple(PrinterAxis.X, -1.0, 3000),
            Triple(PrinterAxis.X, 1.0, 3000),
            Triple(PrinterAxis.X, 10.0, 3000),
            Triple(PrinterAxis.Y, -10.0, 3000),
            Triple(PrinterAxis.Y, -1.0, 3000),
            Triple(PrinterAxis.Y, 1.0, 3000),
            Triple(PrinterAxis.Y, 10.0, 3000),
            Triple(PrinterAxis.Z, -10.0, 900),
            Triple(PrinterAxis.Z, -1.0, 900),
            Triple(PrinterAxis.Z, 1.0, 900),
            Triple(PrinterAxis.Z, 10.0, 900),
        )

        cases.forEach { (axis, deltaMm, feedrate) ->
            val encoded = encode(moveAxisIntent(axis, deltaMm))
            val expectedAxis = axis.name.lowercase()
            assertEquals(
                """{"action":"move_axes","movements":[{"axis":"$expectedAxis","delta_mm":$deltaMm}],"feedrate_mm_per_min":$feedrate}""",
                encoded,
            )
            assertFalse(encoded.contains("required_device_features"))
        }
    }

    @Test fun toggle_light_is_minimal() =
        assertEquals("""{"action":"toggle_light"}""", encode(PrinterControlIntent.ToggleLight))

    @Test fun set_chamber_light() =
        assertEquals(
            """{"action":"set_chamber_light","light_on":true}""",
            encode(PrinterControlIntent.SetChamberLight(on = true)),
        )

    @Test fun set_bed_temperature() =
        assertEquals(
            """{"action":"set_bed_temperature","temperature_celsius":60,"wait":false}""",
            encode(PrinterControlIntent.SetBedTemperature(temperatureCelsius = 60, wait = false)),
        )

    @Test fun set_chamber_temperature() =
        assertEquals(
            """{"action":"set_chamber_temperature","temperature_celsius":40,"wait":true}""",
            encode(PrinterControlIntent.SetChamberTemperature(temperatureCelsius = 40, wait = true)),
        )

    @Test fun set_hotend_temperature_omits_null_extruder() =
        assertEquals(
            """{"action":"set_hotend_temperature","temperature_celsius":220,"wait":true}""",
            encode(PrinterControlIntent.SetHotendTemperature(temperatureCelsius = 220, wait = true)),
        )

    @Test fun set_hotend_temperature_with_extruder() =
        assertEquals(
            """{"action":"set_hotend_temperature","temperature_celsius":220,"wait":true,"extruder_id":0}""",
            encode(PrinterControlIntent.SetHotendTemperature(temperatureCelsius = 220, wait = true, extruderId = 0)),
        )

    @Test fun ams_reread_rfid() =
        assertEquals(
            """{"action":"ams_reread_rfid","ams_id":1,"slot_id":2}""",
            encode(PrinterControlIntent.AmsRereadRfid(amsId = 1, slotId = 2)),
        )

    @Test fun ams_load_minimal() =
        assertEquals(
            """{"action":"ams_load_filament","ams_id":1,"slot_id":2}""",
            encode(PrinterControlIntent.AmsLoadFilament(amsId = 1, slotId = 2)),
        )

    @Test fun ams_load_global_tray() =
        assertEquals(
            """{"action":"ams_load_filament","ams_id":1,"slot_id":2,"global_tray_id":5}""",
            encode(PrinterControlIntent.AmsLoadFilament(amsId = 1, slotId = 2, globalTrayId = 5)),
        )

    @Test fun ams_load_external_id() =
        assertEquals(
            """{"action":"ams_load_filament","ams_id":1,"slot_id":2,"external_id":"ext1"}""",
            encode(PrinterControlIntent.AmsLoadFilament(amsId = 1, slotId = 2, externalId = "ext1")),
        )

    @Test fun ams_unload_extruder() =
        assertEquals(
            """{"action":"ams_unload_filament","ams_id":1,"slot_id":2,"extruder_id":0}""",
            encode(PrinterControlIntent.AmsUnloadFilament(amsId = 1, slotId = 2, extruderId = 0)),
        )

    @Test fun ams_unload_minimal() =
        assertEquals(
            """{"action":"ams_unload_filament","ams_id":1,"slot_id":2}""",
            encode(PrinterControlIntent.AmsUnloadFilament(amsId = 1, slotId = 2)),
        )
}
