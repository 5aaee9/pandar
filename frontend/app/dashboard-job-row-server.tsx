...
export async function JobRowServer({
  job,
  printerName,
  agentName,
  actions,
  ...
  const tMat = await getTranslations('material')
  const tRec = await getTranslations('recovery.state')
  const tJf = await getTranslations('jobFormat')
  const tMonitor = await getTranslations('printMonitor')
