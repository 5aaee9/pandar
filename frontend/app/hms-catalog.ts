...
export function hmsMessage(serialNumber: string, code: string, locale: string) {
  const catalog = catalogs[locale.toLowerCase().startsWith('zh') ? 'zh-cn' : 'en']
  const prefixIndex = catalog.prefixes.indexOf(serialNumber.slice(0, 3).toUpperCase())
  if (prefixIndex === -1) {
    return null
  }

  const messageIndex = catalog.codes[code.toUpperCase()]?.[prefixIndex]
  return typeof messageIndex === 'number' ? catalog.messages[messageIndex] : null
}
