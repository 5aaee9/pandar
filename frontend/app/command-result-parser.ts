...
export function parseCommandResult(command: Command | null): CommandResultData | null {
    ...
    if (parsed.type === 'printer_discovery' && Array.isArray(parsed.printers)) {
      return {
        ...
        printers: parsed.printers
          ...
          .filter((printer): printer is DiscoveryResultData['printers'][number] =>
            Boolean(printer),
          ),
      }
    }
    if (
      ...
      return {
        ...
        checks: parsed.checks
          ...
          .filter((check): check is DiagnosticResultData['checks'][number] => Boolean(check)),
...
function parseDiagnosticCheck(value: unknown): DiagnosticResultData['checks'][number] | null {
  if (
    !isRecord(value) ||
...
function parseCompatibility(value: unknown): DiagnosticResultData['compatibility'] {
  if (!isRecord(value)) {
    return undefined
  }
  ...
  return {
    normalized_model:
      typeof value.normalized_model === 'string' || value.normalized_model === null
